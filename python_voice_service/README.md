...
Unity and the desktop runtime can fetch speech from:

- Piper main: `GET http://127.0.0.1:5005/speak?text=...`
- Kokoro alternate: `GET http://127.0.0.1:5007/speak?text=...`

This repo now ships two compatible HTTP wrappers:
...
- `GET /speak?...` -> `audio/wav`
- `POST /speak` -> base64 WAV JSON

Piper additionally exposes true streaming:

- `GET /speak_stream?text=...&voice=...` -> chunked `audio/L16` (PCM s16le mono) with `X-Audio-Sample-Rate`.

Kokoro additionally exposes:
...
The local launcher supports two independent TTS commands:
...
$env:KOKORO_TTS_SPEED="1.0"
$env:KOKORO_TTS_MAX_TEXT_CHARS="0"
...
Kokoro tuning knobs (optional):

- `KOKORO_TTS_VOICE`: default voice, for example `af_heart`.
- `KOKORO_TTS_LANG_CODE`: default language code, for example `a` (American English).
- `KOKORO_TTS_SPEED`: default playback speed.
- `KOKORO_TTS_CACHE_SIZE`: cache last N request results (set `0` to disable).
- `KOKORO_TTS_MAX_CONCURRENCY`: synthesis concurrency gate.
- `KOKORO_TTS_MAX_TEXT_CHARS`: optional input truncation guardrail.
- `KOKORO_TTS_SAMPLE_RATE`: output sample rate.
- `KOKORO_TTS_SEGMENT_SILENCE_MS`: silence inserted between generated segments.
...
#### Recommended: keep TTS wrappers in a separate venv

The source launcher still works best with a dedicated `.venv_tts` for Piper/Kokoro wrappers and a separate `.venv_asr` for Faster-Whisper / Moonshine / Ollama-related dependencies:

- **ASR/LLM venv** (port 8000): uses `requirements.txt`
- **TTS venv** (ports 5005/5007): uses `requirements_tts.txt`
...
# Local TTS wrappers (Piper/Kokoro)
